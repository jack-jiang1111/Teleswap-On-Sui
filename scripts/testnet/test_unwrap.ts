...
// Test data constants
const LOCKER1_LOCKING_SCRIPT = "0x1976a9144062c8aeed4f81c2d73ff854a2957021191e20b688ac";
...
const BURN_PROOF_DATA = {
  ...
  vout: "0x0300ca9a3b000000001a1976a9144062c8aeed4f81c2d73ff854a2957021191e20b688ac0000000000000000296a2701878799c85d1bcbd9419b150d2e2dabb1cdc49f361944e9235d8204ee45871c2b000003e800000000000005f5e1001a1976a91412ab8dc588ca9d5787dde7eb29569da63c3a238c88ac", 
  locktime: "0x00000000",
  ...
  async testSwapAndUnwrap(): Promise<boolean> {
    ...
    try {
      ...
      if (suiCoins.length > 0) {
        const inputAmount = Math.min(1000000, suiCoins[0].balance); // Use 0.001 SUI or available balance
        amounts = [inputAmount];
        inputCoinIds.sui = [suiCoins[0].coinObjectId];
        console.log(`Using SUI coin: ${suiCoins[0].coinObjectId} with amount: ${inputAmount}`);
      } else if (usdcCoins.length > 0) {
        const inputAmount = Math.min(1000000, usdcCoins[0].balance); // Use 0.001 USDC or available balance
        amounts = [inputAmount];
        inputCoinIds.usdc = [usdcCoins[0].coinObjectId];
        console.log(`Using USDC coin: ${usdcCoins[0].coinObjectId} with amount: ${inputAmount}`);
      } else {
