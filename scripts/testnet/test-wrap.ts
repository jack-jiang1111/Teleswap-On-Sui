...
async function main() {
  ...
  // full lock script
  let LOCKER1_SCRIPT = '0x76a9144062c8aeed4f81c2d73ff854a2957021191e20b688ac';
  let LOCKER1_SCRIPT_TYPE = 4; // P2PKH

  // 20 bytes pubkey hash
  let LOCKER1_PUBKEY_HASH =        '0x4062c8aeed4f81c2d73ff854a2957021191e20b6';

  // this also simulate the user script
  ...
  try {
    const reqRes = await sdk.requestToBecomeLocker({
      ...
      lockerLockingPubHashHex: LOCKER1_SCRIPT,
      lockerScriptType: LOCKER1_SCRIPT_TYPE,
      lockerRescuePubHashHex: LOCKER_RESCUE_SCRIPT_P2PKH,
    });
  ...
  try {
    const wrapRes = await sdk.wrap({
      ...
      voutHex: '0x0300ca9a3b000000001976a9144062c8aeed4f81c2d73ff854a2957021191e20b688ac0000000000000000296a2701878799c85d1bcbd9419b150d2e2dabb1cdc49f361944e9235d8204ee45871c2b000003e8000000e1f505000000001976a91412ab8dc588ca9d5787dde7eb29569da63c3a238c88ac',
      locktimeHex: '0x00000000',
      blockNumber: 497,
      intermediateNodesHex: '0x5ff6a258853ed3a4dd9fff062df80397ee4f8c7082c534ade5645741da01a848',
      index: 1,
      lockerLockingScriptHex: LOCKER1_SCRIPT,
    });
