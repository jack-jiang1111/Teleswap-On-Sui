...
module teleswap::lockerstorage {
    ...
    public struct NewCollateralTokenEvent has copy, drop {
        token: address,
        decimal: u8,
    }
    ...
    public fun initialize(
        ...
        let lib_constants = LockersLibConstants {
            ...
            native_token_decimal: NATIVE_TOKEN_DECIMAL,
    ...
    public fun add_collateral_token(
        ...
        assert!(_collateral_token != @0x0, ERROR_ZERO_ADDRESS);
        
        // Add collateral token decimal to the table (key: token address, value: decimal)
        table::add(&mut locker_cap.locker_collateral_decimal, _collateral_token, _collateral_decimal);
        
        // Emit NewCollateralTokenEvent
        event::emit(NewCollateralTokenEvent {
            token: _collateral_token,
            decimal: _collateral_decimal,
        });
    }

    /// @notice Sets locker collateral token
    /// @param locker_cap The locker capability object
    /// @param locker_target_address The target address of the locker
    /// @param collateral_token The collateral token address to set
    public(package) fun set_locker_collateral_token(
        locker_cap: &mut LockerCap,
        locker_target_address: address,
        collateral_token: address,
    ) {
        // Update the locker collateral token mapping (overwrites if key exists)
        if (table::contains(&locker_cap.locker_collateral_token, locker_target_address)) {
            table::remove(&mut locker_cap.locker_collateral_token, locker_target_address);
        };
        table::add(&mut locker_cap.locker_collateral_token, locker_target_address, collateral_token);
    }
    ...
    /// @param locker_cap The locker capability object
    /// @return The inactivation delay
    public fun get_inactivation_delay(locker_cap: &LockerCap): u64 {
        // For now, return a default value. This should be stored in lib_constants
        // TODO: Add INACTIVATION_DELAY to LockersLibConstants
        86400 // 24 hours in seconds (default)
    }
    ...
    /// @param ctx Transaction context
    /// @return True if locker is active
    public fun is_locker_active(locker_cap: &LockerCap, _locker_target_address: address, ctx: &TxContext): bool {
    ...
    /// @notice Gets the collateral token address for a locker
    /// @dev This function returns the address of the collateral token used by a specific locker
    /// @param locker_cap The locker capability object
    /// @param locker_target_address The target address of the locker
    /// @return The address of the collateral token
    public fun get_collateral_token(locker_cap: &LockerCap, locker_target_address: address): address {
        assert!(table::contains(&locker_cap.locker_collateral_token, locker_target_address), ERROR_INVALID_GET);
        *table::borrow(&locker_cap.locker_collateral_token, locker_target_address)
    }

    /// @notice Gets the decimal places for a collateral token
    /// @dev This function returns the number of decimal places for a specific collateral token
    /// @param locker_cap The locker capability object
    /// @param collateral_token The address of the collateral token
    /// @return The number of decimal places for the collateral token
    public fun get_collateral_decimal(locker_cap: &LockerCap, collateral_token: address): u8 {
        assert!(table::contains(&locker_cap.locker_collateral_decimal, collateral_token), ERROR_INVALID_GET);
        *table::borrow(&locker_cap.locker_collateral_decimal, collateral_token)
    }
    ...
    public fun calculate_health_factor(
        ...
        let numerator = price_of_one_unit_of_collateral * 
                       the_locker.collateral_token_locked_amount * 
                       one_hundred_percent * 
                       (10 * one_hundred_percent * one_hundred_percent);
        
        let denominator = the_locker.net_minted * 
                         liquidation_ratio * 
                         _reliability_factor * 
                         std::u64::pow(10, 1 + _collateral_decimal);
    ...
    public fun needed_telebtc_to_buy_collateral(
        ...
        let denominator = locker_cap.lib_constants.one_hundred_percent * std::u64::pow(10, _collateral_decimal );
        numerator / denominator
    }
    ...
    /// @param _collateral_token Address of the collateral token
    /// @param _collateral_decimal Decimal places of the collateral token
    /// @param locker_cap The locker capability object
    /// @return Price in BTC
    public fun price_of_one_unit_of_collateral_in_btc(
        ...
        // Check if collateral decimal is not zero (whitelisted)
        assert!(_collateral_decimal != 0, ERROR_ZERO_VALUE);
        ...
        price_oracle::equivalent_output_amount(
            std::u64::pow(10, _collateral_decimal), // 1 unit of collateral
            _collateral_decimal,
            8, // TeleBTC decimal
            _collateral_token,
            @0x0 // telebtc address ????
    ...
    public fun get_locker_capacity(
        ...
        let locker_collateral_in_telebtc = price_of_one_unit_of_collateral * 
                                          the_locker.collateral_token_locked_amount * 
                                          locker_cap.lib_constants.one_hundred_percent * 
                                          locker_cap.lib_constants.one_hundred_percent / 
                                          (locker_cap.collateral_ratio * _locker_reliability_factor * std::u64::pow(10, _collateral_decimal));
    ...
    public fun maximum_buyable_collateral(
        ...
        let antecedent = ((upper_health_factor * 
                          ...
                          liquidation_ratio * 
                          _reliability_factor * 
                                                     std::u64::pow(10, _collateral_decimal)) / one_hundred_percent) - 
                        (the_locker.collateral_token_locked_amount * 
    ...
    /// @notice Removes the collateral token mapping for a locker
    /// @param locker_cap The locker capability object
    /// @param locker_target_address The locker target address
    public(package) fun remove_locker_collateral_token(
        locker_cap: &mut LockerCap,
        locker_target_address: address
    ) {
        table::remove(&mut locker_cap.locker_collateral_token, locker_target_address);
