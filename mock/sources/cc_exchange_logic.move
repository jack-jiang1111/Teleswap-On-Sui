...
module teleswap::cc_exchange_logic {
    ...
    /// Mints TeleBTC and calculates protocol/locker/third party fees. Updates the request with fee breakdown
    ...
    /// - Calculates protocol fee, third-party fee (based on `third_party` id and current table entry),
    ...
    /// - Updates the request fields in-place: protocol_fee, third_party_fee, locker_fee, and
    ...
    fun execute_wrap_and_swap(
        ...
        if(result) {
            ...
            send_locker_fee(storage, tx_id, locker_locking_script, locker_fee_coin, locker_cap, ctx);
    ...
    fun send_fees(
        ...
        send_locker_fee(storage, tx_id, locker_locking_script, remaining_coin, locker_cap, ctx);
    ...
    fun send_locker_fee(
        ...
        if (reward_distributor == @0x0 || third_party != 0) {
            ...
            // TODO: Implement reward distributor call
            // For now, just send directly to locker
            transfer::public_transfer(locker_fee_coin, locker_address);
        };
    }
