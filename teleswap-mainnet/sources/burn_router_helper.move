...
module teleswap::burn_router_helper {
    ...
    /// @notice Test function with hardcoded vout data from terminal selection
    /// @dev Tests the parse_value_having_locking_script function with the specific vout data provided
    /// @return The parsed amount from the hardcoded vout
    #[test]
    public fun test_parsed_amount_hardcoded(): u64 {

        let wrap_vout = x"0300ca9a3b000000001976a91412ab8dc588ca9d5787dde7eb29569da63c3a238c88ac0000000000000000296a2701878799c85d1bcbd9419b150d2e2dabb1cdc49f361944e9235d8204ee45871c2b000003e8000000e1f505000000001976a9144062c8aeed4f81c2d73ff854a2957021191e20b688ac";
        let locker_script = x"a9144062c8aeed4f81c2d73ff854a2957021191e20b688ac";
     let (input_amount, arbitrary_data) = bitcoin_helper::parse_value_and_data_having_locking_script_small_payload(
            &wrap_vout,
            &locker_script
        );

        std::debug::print(&input_amount);
        std::debug::print(&arbitrary_data);
        // Return the parsed amount for verification
        input_amount
    }
