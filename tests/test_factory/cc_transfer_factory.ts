...
function updateMoveToml(btcrelayPackageId: string) {
    // Update mock/Move.toml
    const TeleswapMainPackageMoveTomlPath = path.join(__dirname, '../../mock/Move.toml');
    ...
    console.log('Updated mock/Move.toml with actual package IDs');
...
export async function CCTransferFactory() {
    ...
    tx.setGasBudget(500000000);
    tx.moveCall({
        target: `${ccTransferRouterPackageId}::btcrelay::initialize`,
        arguments: [
    ...
    // 4. Initialize Locker
    let lockerResult = await initializeLocker(ccTransferRouterPackageId,lockerAdminCapabilityId);
